...
const Competitions = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://34.106.176.23:5000/list-entered-competitions/${getUser().uid}`)
      .then(res => {
        return res.json()
      })
      .then(result => {
    ...
    fetch(
      `http://34.106.176.23:5000/list-nonregisted-competitions/${getUser().uid}`
    )
      .then(res => {
        return res.json()
      })
      ...
      {enteredComps.length > 0 ? (
        <Box sx={{ width: 1 }}>
          ...
          <Button
            ...
            className="mdc-button mdc-button--raised"
            onClick={event => {
              navigate(`/app/enteredcompetitions`)
            }}
          >
            {" "}
            See More
